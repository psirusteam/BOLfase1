...
consulta_asistencEscolar <- consulta_asistencEscolar %>% 
  mutate(rezago_escolar = as.numeric(P25_EDAD >= 6 & P25_EDAD <= 21 & P36_ASISTE == 1 &        
...
Esta carencia se considera si los Hogares donde no se recibe atención médica, o se atienden en
farmacias, medicina tradicional, o dependencias de ONG’s o Iglesia. Las variables a considerar son:
...
consulta_salud <- personas %>% select(I_BC_VIV, ID_BC_PERS, P28A_CAJA, P28B_SEGPRIV,
  ...
  collect() %>%
  mutate(CARENCIA_SALUD = ifelse(P28A_CAJA == 2 & P28B_SEGPRIV == 2 & P28C_PUBLIC == 2 &
                                  P28D_PRIVAD == 2, 1, 0)) %>% 
  select(I_BC_VIV, CARENCIA_SALUD)
consulta_salud <- consulta_salud %>% group_by(I_BC_VIV) %>% summarise(CARENCIA_SALUD = sum(CARENCIA_SALUD)) %>%
...
consulta_ocupXpers <- personas %>% select(I_BC_VIV, P25_EDAD, P39_TRABAJO,
                                          P41_BUSCO) %>% collect() %>% 
                      dplyr::mutate(ocupado = as.numeric((P25_EDAD >= 15 & P39_TRABAJO == 1) | 
                      (P25_EDAD >= 15 & P39_TRABAJO == 2 & P41_BUSCO %in% 1:2))) %>%
    group_by(I_BC_VIV) %>%
  summarise(total_ocupados = sum(ocupado, na.rm = T), numpers_hog = n()) %>% ungroup() %>%
...
consulta_precariedadOcupacional <- consulta_precariedadOcupacional %>% 
  mutate(CARENCIA_PRECARIEDADOCUPACIONAL= as.numeric(ocupXpers <= 1/4 & jefehogNOeduc == 1)) %>% 
