...
Los cálculos de los indicadores del NBO se basan en el siguiente documento: 
...
El indicador anterior se calcula a nivel de hogar, si al menos un miembro del hogar presenta carencia el hogar se considerará con carencia:
...
gc(reset = T)
consulta_ocupXpers <- personas %>% select(I_BC_VIV, P25_EDAD, P39_TRABAJO,
                                          P41_BUSCO) %>% collect() %>% 
                      dplyr::mutate(ocupado = as.numeric((P25_EDAD >= 15 & 
                                                          P39_TRABAJO == 1) | 
                      (P25_EDAD >= 15 & 
                       P39_TRABAJO == 2 & 
                       P41_BUSCO %in% 1:2))) %>%
    group_by(I_BC_VIV) %>%
  summarise(total_ocupados = sum(ocupado, na.rm = T), 
            numpers_hog = n()) %>%
  ungroup() %>%
...
consulta_hogar_nbi$CARENCIA_ASISESCOLAR <- ifelse(is.na(consulta_hogar_nbi$CARENCIA_ASISESCOLAR), 1, 0)
...
```{r}
consulta_ocupac <- personas %>% select(I_BC_VIV, ID_BC_PERS, URBRUR_P, P25_EDAD, P39_TRABAJO, P40_QUEHIZO,  P41_BUSCO) %>% 
  collect() %>%  
  mutate(PET = ifelse(URBRUR_P %in% 1:2 & P25_EDAD >= 15, 1, 0)) # Población edad de trabajar


consulta_ocupac <- consulta_ocupac %>% 
  mutate(PEA = as.numeric(PET == 1 & 
                            (P39_TRABAJO == 1 | # Trabajaron  
                            (PET == 1 & P39_TRABAJO == 2 & P40_QUEHIZO %in% 1:4) |
...
consulta_ocupac <- consulta_ocupac %>% mutate(Desocupados = as.numeric(PEA == 1 & P39_TRABAJO == 2 & (P41_BUSCO %in% c(1, 2))),
                            Ocupados = as.numeric(PEA == 1 & (P39_TRABAJO == 1 |
                                                (P39_TRABAJO == 2 & 
                                                   P40_QUEHIZO %in% 1:4))
                                                  )) # No trabajo

consulta_ocupac <-  consulta_ocupac %>% select(I_BC_VIV, ID_BC_PERS, PET, PEA, Desocupados, Ocupados) 
consulta_ocupac$PEA <- ifelse(consulta_ocupac$PET == 0, NA, consulta_ocupac$PEA)
consulta_ocupac$Desocupados  <- ifelse(consulta_ocupac$PET == 0, NA, consulta_ocupac$Desocupados)
consulta_ocupac$Ocupados  <- ifelse(consulta_ocupac$PEA == 0, NA, consulta_ocupac$Ocupados)
```
